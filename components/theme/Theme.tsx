...
const ThemeProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const [currentMusic, setInitialApp] = useAppStore((state) => [
    state.currentMusic,
    state.setInitialApp,
  ]);
  ...
  useEffect(() => {
    ...
    if (!localStorage.getItem(process.env.NEXT_PUBLIC_APP_STORE)) {
      ...
      const appItem: AppLocalStorage = JSON.parse(
        localStorage.getItem(process.env.NEXT_PUBLIC_APP_STORE)
      );
  ...
  useEffect(() => {
    ...
    if (!token) {
      if (localStorage.getItem(process.env.NEXT_PUBLIC_AUTH_STORE)) {
        const authItem: AuthLocalStorage = JSON.parse(
          localStorage.getItem(process.env.NEXT_PUBLIC_AUTH_STORE)
        );
        const currentTime = new Date().getTime();
        const setTime = new Date(
          authItem.set_at +
            Number(process.env.NEXT_PUBLIC_AUTH_TOKEN_CREDIT) * 3600000
        ).getTime();
  ...
  }, [token, user, login, setInitialAuth, userId]);
