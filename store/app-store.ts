  ...
  if (typeof window !== "undefined") {
    const item: AppLocalStorage = JSON.parse(
      localStorage.getItem(STORAGE_NAME) ||
        '{"currentMusic":null,"isPlaying":false,"playList":[],"playListId":"","volume":1,"currentMusicTime":0,"repeatType":"all","shuffleIndex":[]}'
    );
...
export const useAppStore = create<AppStoreState>()((set) => ({
  ...
  setDisableKeyDown: (disabled: boolean) =>
    set(() => ({ disableKeyDown: disabled })),
  ...
  changeMusic: (type: ChangeMusicType, auto?: boolean) => {
    ...
    let currentMusicIndex = playList.findIndex(
    ...
    if (repeatType === "shuffle") {
      let shuffleIndex = useAppStore
        .getState()
        .shuffleIndex.findIndex((indexes) => indexes === currentMusicIndex);
